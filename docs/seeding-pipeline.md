...
This is the historical record of how PrayProMax bootstrapped from `ref.md`
(a curated list of ~245 living mystical / religious traditions with prayer
practice) to **291 seeded traditions + 263 virtual officiant cards** in
production. A community contributor running their own slice would follow the
same pattern.

## Phase A — `ref.md` → `INDEX.yaml v0`

**Goal**: parse the source survey into a structured categorized list.

- Tool: Claude subagent
- Action: classify each `ref.md` bullet into the 18-category enum, generate
...
**Goal**: catch ref.md gaps / misclassifications / spurious entries / bad ids.

- Tool: 3 audit workers (Claude / Codex / Grok)
- Action: each independently audits ref.md + INDEX v0, produces YAML report
...
- Output: `INDEX.yaml v1` — **345 entries** (291 pending + 54 skipped)

## Phase C — worker prompt templates

**Goal**: define what each seeding-worker produces.

- 4 prompts in `scripts/seed-prompts/`:
  ...
  - `tradition-worker-api.md` / `tradition-merge-api.md` (Phase E
    variants using sub2api HTTP API instead of codex CLI)
...
- Pipeline: gpt-5.5 (sub2api) + grok-4.20-fast + gpt-5.5 merge, parallel=10
- Wall time: ~57 minutes for 291 traditions
- First failure mode: ChatGPT subscription rate-limited at ~4 traditions in
  → switched from codex CLI to sub2api HTTP (gpt-5.5 via proxy)
...
- Officiant generation: separate batch via `scripts/dispatch-officiants.py`
  → 263 cards in 13 min, 0 errors
- Review pass: `scripts/review-seeded.py` →
  - 291/291 traditions: 0 parse errors, 0 missing fields, 0 wish_type gaps,
    0 forbidden phrases, 0 fence/preamble residue
  - 263/263 officiants: 0 errors after a tiny 2-card cleanup
...
| `sub2api` HTTP (gpt-5.5)  | Phase E main      | bulk seeding + merge                       |
...
1. Fork repo + clone
2. Set env vars (or edit `scripts/sub2api.py` keys) with their own gpt key
...
6. Open PR
7. CI review pipeline (3 reviewers + schema + lineage gates) runs
8. Auto-merge or human review based on gate decisions
